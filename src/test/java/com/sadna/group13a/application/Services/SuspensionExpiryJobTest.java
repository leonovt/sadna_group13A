...
class SuspensionExpiryJobTest {
    ...
    void givenSuspendedUserWithFutureExpiry_whenJobRuns_thenNotLifted() {
        ...
        when(suspended.getSuspendedUntil()).thenReturn(LocalDateTime.now().plusDays(1));
    ...
    @Test
    void givenSuspendedUserWithPastExpiry_whenJobRuns_thenLiftedAndEventsPublished() {
        User expired = mock(User.class);
        when(expired.isSuspended()).thenReturn(true);
        when(expired.getSuspendedUntil()).thenReturn(LocalDateTime.now().minusDays(1));
        ...
        verify(systemLogService).logEvent(any());
    }

    @Test
    void givenMixOfExpiredAndFutureSuspensions_whenJobRuns_thenOnlyExpiredLifted() {
        User expired = mock(User.class);
        when(expired.isSuspended()).thenReturn(true);
        when(expired.getSuspendedUntil()).thenReturn(LocalDateTime.now().minusDays(1));
        when(expired.getId()).thenReturn("user-1");
        ...
        when(future.getSuspendedUntil()).thenReturn(LocalDateTime.now().plusDays(1));
        ...
        verify(userRepository, never()).save(future);
        verify(userRepository, times(1)).save(any());
    }

    @Test
    void givenRepositorySaveThrows_whenJobRuns_thenExceptionNotPropagated() {
        User expired = mock(User.class);
        when(expired.isSuspended()).thenReturn(true);
        when(expired.getSuspendedUntil()).thenReturn(LocalDateTime.now().minusDays(1));
        when(expired.getId()).thenReturn("user-1");
